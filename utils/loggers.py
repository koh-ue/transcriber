...
class Log:
    def __init__(self, _name, _file, no_log_file, config_file="src/library/misc/log-config.yaml") -> None:
        ...
        if no_log_file:
            ...
            self.log_conf["handlers"]["fileHandler"]["filename"] = f"{os.path.dirname(_file)}/logs/{os.path.splitext(os.path.basename(_file))[0]}.log"
            os.makedirs(os.path.dirname(self.log_conf["handlers"]["fileHandler"]["filename"]), exist_ok=True)
        config.dictConfig(self.log_conf)
        self.logger = getLogger(_name)